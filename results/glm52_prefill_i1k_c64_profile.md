...
- **Attention (0.67x, dense FA 27 vs 9.9): fp8 batch-prefill is a real ~9–10 ms fork lever (PROTOTYPED, pending GSM8K).** The bf16 ck_tile FA is occupancy-limited by the `[BM,256]` fp32 online-softmax accumulator in VGPRs (B200 holds it in TMEM → ~96% of ~2.25 PFLOPS bf16 vs MI355X ~14% util). Two fork-side experiments:
  ...
  - *fp8 batch-prefill (IMPLEMENTED, env-gated `SGLANG_DSA_FP8_DENSE_ATTN`, default off, commit `c410a22111`): accuracy-neutral ~13% FA win — viable.* `aiter.mha_batch_prefill_func` takes fp8 q/k/v + descales. Microbench at the GLM shape (page_size=16, the only D=256-supported page size; 32/64/128/256 → `no matching kernel`): **ck_tile bf16 1.526 ms → mha_batch_prefill fp8 1.334 ms = 1.14x** on the FA kernel → ≈4–5 ms off the 39.2 ms subtotal. **No free cast-skip for our weights:** GLM-5.2's `kv_b_proj`/`q_b_proj`/`o_proj` are in the quark `exclude` list (**bf16**, not MXFP4/uint8), so the fused `fused_gemm_afp4wfp4_split_cat` fp8-K/V path (`forward_mha.py:299`, gated on uint8) never fires; we quantize the decompressed bf16 K/V/Q to fp8 in `_forward_standard_mha_fp8` and rebuild the varlen K/V into the page_size=16 paged layout (ragged sequences scattered into padded pages, masked via `kv_last_page_lens`; paging verified correct vs bf16 ref across uniform/ragged/sub-page batches). **Accuracy A/B (full 1319-q GSM8K, same box): fp8 dense = 0.928 vs bf16 dense = 0.929 → parity** (Δ ≈ 1 question; the earlier 200-q 0.930-vs-0.955 gap was noise). The compiled kernel is per-tensor descale only at D=256, and per-tensor fp8 is sufficient here. B200 side `fmhaSm100f...` is a trtllm-gen precompiled cubin (no portable source).
