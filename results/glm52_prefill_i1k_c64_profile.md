...
- **Where MI355X actually trails B200 at i1k/conc64:** attention (0.67x — dense FA 27 vs 9.9, B200's SM100 fmha is far faster) and MoE (0.78x). Within MoE the gate-up GEMM is at parity (31.1 vs 32.1).
  ...
  - **Expert combine (`moe_reduction` 22.3 vs `finalizeKernelVecLoad` 15.8): the addressable candidate.** The plain-bf16 top-9 reduction looks a little slow. Caveat: B200 splits its combine across `finalizeKernelVecLoad` (15.8) + a routed-scale/shared-add (`CUDAFunctor_add` ~5.4), so the *fair* combine gap is smaller than 1.41x — but the MI355X reduction is worth a look (fuse routed-scale into the reduction, better vectorization).
