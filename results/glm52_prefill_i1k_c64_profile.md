...
- **Attention (0.67x, dense FA 27 vs 9.9): AMD FA wall at head_dim 256, not a fixable ck_tile pick.** Same bf16/head_dim-256/causal FA both sides; B200 runs its fmha near peak (~96% of ~2.25 PFLOPS bf16), MI355X's FA lands at only ~14% util. **Tested** the fork lever (route through the gfx950-tuned triton `extend_attention_fwd`): microbench at the exact GLM shape ([65536,16,256], 64×1024 causal) gives **ck_tile 1.525 ms vs tuned-triton 1.511 ms = 1.01x** (correct, maxdiff 0.008). So both AMD FA kernels hit the **same** head_dim-256 occupancy wall — a backend swap reclaims nothing. Mechanism = head_dim 256 (register/SMEM pressure, low wave occupancy; AMD FA kernels are tuned for 64/128). Remaining levers are not fork-side: an **fp8 dense FA** (none exists on this path) or a fundamentally better AMD head_dim-256 FA kernel (aiter/upstream).
