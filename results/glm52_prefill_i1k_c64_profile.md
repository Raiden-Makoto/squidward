...
- **MoE (0.68x):** fresh tuned capture (upstream `glm5_fp4` tiles, confirmed executing).
  - **Down GEMM (46.7 vs 26.2, 0.56x) — overhead/latency-bound, NOT bandwidth- or compute-roofline-bound.** rocprofv3 on the real production forward (`bench_one_batch`, batch16/i1k → ~16.4k tok/fwd, per-expert M≈573; kernel `mfma_moe2 cshuffle t64x128x256 reduce sbm128`): `MemUnitStalled 0.58%` (so *not* HBM-BW-bound — corrects the earlier "memory-bound" reading), `MfmaUtil 13%` (matrix cores idle ~87%), OccupancyPercent 37%, VGPR 64, LDS 32.5 KB. It's a grouped-GEMM latency/overhead regime — MXFP4 dequant/scale + LDS/pipeline-fill dominate over matrix throughput; ~13% of the ~10 PFLOP/s MXFP4 peak. (Stage-1 gate-up `t128x128x256` is even more occupancy-crushed: occ 9.7%, LDS 80 KB.) B200's cutlass grouped `bmm` schedules this regime far better.
...
1. **MoE down-GEMM efficiency** — gemm2 46.7 vs B200 26.2 (the ~20 ms prize). rocprofv3 verdict: NOT bandwidth-bound (`MemUnitStalled 0.1%`) and NOT compute-roofline-bound (`MfmaUtil 14.5%`, ~13% of MXFP4 peak) — it's a small grouped-GEMM overhead/latency regime (MXFP4 dequant/scale + LDS/pipeline-fill dominate; occupancy 33%). Lever is a better aiter grouped kernel: deeper software pipelining + higher MFMA overlap of dequant with matmul + persistent-CTA scheduling to raise util, not a tile pick and not re-tuning. B200's cutlass grouped `bmm` schedules this regime far better. Hard, aiter/upstream (verify at production per-expert M via `bench_one_batch`/live-server trace before implementing).
