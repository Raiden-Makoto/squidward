...
- **Where MI355X actually trails B200 at i1k/conc64:** attention (0.67x — dense FA 27 vs 9.9, B200's SM100 fmha is far faster) and MoE (0.78x). Within MoE the gate-up GEMM is already at parity (31.1 vs 32.1); the two real MoE gaps are the **down GEMM** (`mfma_moe2 t64x256` 41.4 vs 26.2, 1.58x — tuned but still behind; likely near the CDNA4 mxfp4 ceiling like the dense GEMMs) and the **expert combine** (`moe_reduction` 22.3 vs `finalizeKernelVecLoad` 15.8, 1.41x — B200 fuses the routed-scale into finalize, so fusing scale+combine is a candidate lever). all-reduce / dense-GEMM / RMSNorm / router are at/above parity.
