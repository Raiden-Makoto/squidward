...
- **Router GEMM: MI355X runs it standalone (~4.4 ms/fwd), B200 fuses it — not a clean anomaly.** MI355X computes the MoE gate as a standalone bf16 GEMM (`aiter_dsv3_router_gemm` → `tgemm.mm` → `hgemm_bf16_128x128x64x4_SPK1` 16.3 ms + `MT32x32x512` helper 1.2 ms over the trace; ≈4.4 ms/forward; shape M×6144→256, K=6144). On B200 the gate matmul is **fused into another GEMM** — only the split-K reduction (`cublasLt::splitKreduce`, ~50 µs/fwd) shows under the gate; the main matmul is folded into the nvjet dense-GEMM bucket. So B200 is **not** doing the router for free; its cost is just not itemized, so "MI355X router is anomalously slow vs B200" is **not** a fair claim. Open question / potential lever: is MI355X's standalone gate genuinely inefficient (poor `SPK1` pick for the skinny-N/large-K shape — the decode path picks `SPK8`) or simply **unfused**? Needs a fair test — force split-K on the router shape and/or fuse the gate into an adjacent GEMM, then measure. Not proven either way yet.
