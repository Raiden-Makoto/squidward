...
- **MoE (0.68x):** fresh tuned capture (upstream `glm5_fp4` tiles, confirmed executing).
  - **Down GEMM (46.7 vs 26.2, 0.56x) — latency/dependency-bound (under-pipelined), NOT bandwidth-, compute-, or dequant-throughput-bound.** rocprofv3 on the real production forward (`bench_one_batch`, batch16/i1k → ~16.4k tok/fwd, per-expert M≈573; kernel `mfma_moe2 cshuffle t64x128x256 reduce sbm128`): MfmaUtil **13%**, VALUBusy **26%**, MemUnitStalled **0.6%** (write ≈1.77 GB/dispatch ≈35% of 8 TB/s; if it were write-bound it'd finish 2.8x faster), OccupancyPercent **37%**, VGPR 64, LDS 32.5 KB. No unit is saturated — the load→fp4-dequant/scale→MFMA→cshuffle→write phases run as a serial dependency chain with only ~3 effective waves to overlap them. LDS (32.5 KB) and VGPR (64) both allow ~8 waves, and the launch is a *full* grid (`grid=(hidden/tile_n=48, #expert-blocks, 1)`, block=256) — so 37% is not a grid or resource wall. Root cause: the stage-2 kernel (`compile_mixed_moe_gemm2` in `mixed_moe_gemm_2stage.py` — the fp4/fp8 dispatch at `moe_kernels.py:459`; NOT `compile_moe_gemm2`, which is the bf16×int4 path) is a **2-stage ping-pong pipeline with a per-tile** `gpu.barrier()`; at ~3 effective waves the barrier + dependency chain isn't hidden. **Pipeline-deepening lever is INFEASIBLE at the deployed shape (tested).** On TP4 the MoE `inter_dim` is row-parallel sharded to 512/rank, so the down-GEMM K=512 and with `tile_k=256` there are only **nk=2 K-tiles/rank** (confirmed live: worker compiles `t64x128x256 pipe_deep=False S=2 nk=2`). A 2-tile loop cannot be deepened. A 3-stage prefetch (`SGLANG_MOE2_PIPE=3`, aiter fork `glm52-moe2-pipe3`) helps only at the *unsharded* nk=8 shape (-4.7%, an off-deployment measurement); at the real per-rank nk=2 it is inert (884→900us = noise). At nk=2 the mainloop is tiny and the kernel is **overhead/epilogue-bound**, coupling it to the combine HBM round-trip → the real lever is **combine fusion (see gap 2)**, not K-pipelining. (Stage-1 gate-up `t128x128x256` *is* LDS-capped: occ 9.7%, LDS 80 KB → 2 wg/CU.) B200's cutlass grouped `bmm` pipelines this regime far better.
...
1. **MoE down-GEMM efficiency — CLOSED as a standalone-kernel target (nk=2).** gemm2 46.7 vs B200 26.2. Original verdict (under-pipelined, MfmaUtil 13%, occ 37%) is correct, but the mainloop levers are exhausted: **per-rank K=512 (TP4 shard) ⇒ only nk=2 K-tiles**, so pipeline-deepening is physically impossible (confirmed inert: `SGLANG_MOE2_PIPE=3` = 884→900us noise at nk=2; the +? "-4.7%" only appears at the unsharded nk=8 shape and does not deploy). Also tested + rejected: full-K-resident LDS = +31%, `cu_num_mul=3` = wash. At nk=2 the kernel is overhead/epilogue-bound and is really the *write* half of the gemm2→HBM→combine round-trip → the remaining lever is combine fusion (item 2), which absorbs the down-GEMM tail. (Kernel: `compile_mixed_moe_gemm2`/`mixed_moe_gemm_2stage.py`, fp4 dispatch `moe_kernels.py:459`.)
2. **Combine overlap / gemm2-combine fusion — ACTIVE lever (was deferred).** combine 22.6 vs B200 15.8 (~7 ms) PLUS the down-GEMM epilogue overhead. aiter's `moe_gather_reduce` is already the same 128-bit vectorized one-block-per-token f32 gather-reduce (no atomics) as B200's `finalizeKernelVecLoad`, ~66% HBM BW standalone — so it is NOT a per-kernel porting gap. B200 hides the finalize behind the gemm2 tail via **PDL** (`cudaGridDependencySynchronize`); CDNA4 has no PDL primitive, so the equivalent is a **persistent producer-consumer megakernel** (gemm2 producer signals per-tile completion; combine consumer spins on a global flag / uses `s_wait`/atomics to start reducing finished tiles). This is now the primary MoE lever — targeting the combined gemm2+combine ~69 ms vs B200 ~42 ms.
