...
1. **MoE down-GEMM efficiency** — gemm2 46.7 vs B200 26.2 (the ~20 ms prize). rocprofv3 at production M≈573 verdict: **latency/dependency-bound, under-pipelined** — MfmaUtil 13%, VALUBusy 26%, MemUnitStalled 0.6%, occ 37%; no unit saturated. Not BW-bound, not compute-bound, not dequant-throughput-bound. Occupancy (37% ≈ 3 waves) is not resource-capped (LDS 32.5 KB, VGPR 64 both allow ~8) and the grid is full (`(hidden/tile_n=48, #expert-blocks, 1)`, block=256) → the limiter is the stage-2 kernel's **2-stage ping-pong pipeline + per-tile** `gpu.barrier()` (`compile_mixed_moe_gemm2` in `mixed_moe_gemm_2stage.py`; fp4/fp8 dispatch at `moe_kernels.py:459` — NOT `compile_moe_gemm2`/`moe_gemm_2stage.py`, which is bf16×int4), which doesn't hide the load→dequant→MFMA→write chain at ~3 waves. Lever: deepen the pipeline (2→3/4-stage prefetch, keeping the ping-pong overlap) to overlap fp4 dequant/scale with MFMA; carry in the aiter fork. Tested + rejected: full-K-resident LDS (collapse barriers) = +31% regression (overlap > barrier count); `cu_num_mul=3` occupancy = wash. B200's cutlass grouped `bmm` pipelines this regime far better.
