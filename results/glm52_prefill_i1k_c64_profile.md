...
- **Anomaly: the router GEMM costs 4.1 ms/forward for no good reason.** Most dense GEMMs are at parity, but the MoE router/gate GEMM (`aiter_dsv3_router_gemm` → `tgemm.mm`, shape M×6144→256, K=6144) picks `hgemm_bf16_128x128x64x4_`**`SPK1`** (no split-K) on MI355X → **4.1 ms/forward**, only ~5% of bf16 roofline (the op is ~0.2 ms of actual FLOPs). B200 runs the same gate as a `cublasLt` **split-K** gemm — negligible, folded into the dense-GEMM bucket. Root cause (matches the 64-vs-32 wave hypothesis): for a skinny-N (256), large-K (6144) GEMM the K reduction must be split for parallelism; MI355X's gfx950 **wave64** 128×128 SPK1 tile serializes K and wastes the 128-wide tile on N=256, whereas B200's **warp32** split-K path parallelizes it. Lever: dispatch the router shape through a split-K solution — the decode path already picks `SPK8`, but prefill wrongly picks `SPK1`; forcing split-K should reclaim most of the 4 ms.
