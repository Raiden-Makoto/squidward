...
GPU-busy per prefill forward: MI355X ~375 ms vs B200 331 ms → 0.88x. All kernel times are **ms per prefill forward pass** (graphs-off eager, dense-MHA at i1k, `--mem-fraction-static 0.85 --chunked-prefill-size 131072`). MI355X = **fresh capture** on the `v0.5.15-rocm720-mi35x-20260711` image (`run_glm52.sh --profile`, upstream `glm5_fp4_tuned_fmoe.csv`, INT4 QuickReduce). **All-reduce row is carried over from the prior clean capture** — the fresh graphs-off capture's QuickReduce two-shot is profiler-perturbed (spin-wait, ~19x); clean re-capture pending. B200 = apples-to-apples capture (GLM-5.2-NVFP4, tp4 modelopt_fp4, kv fp8_e4m3, trtllm DSA, flashinfer_trtllm MoE, dense `fmhaSm100f...H256...Causal`).
...
| Attention | `set_mla_kv_buffer` + concat/cast (KV-cache write) | 8.7 | `set_mla_kv_buffer` + `float8_copy` + concat `direct_copy` | 11.0 | |
| Attention | `fused_qk_norm_mrope` (q/k norm + rope, fused) | 1.3 | flashinfer `LayerNorm`+`RMSNorm` + `fused_rope_kernel` | 4.2 | |
| Attention | DSA indexer (`fast_hadamard` + `indexer_k_quant`) | 0.3 | `fast_hadamard` + `fused_store_indexer_cache` | 1.1 | |
| **Attention subtotal** | | **37.3** | | **26.3** | **0.71x** |
| MoE gate-up GEMM (gemm1) | `flydsl_moe1_afp4_wfp4` stage1 | 38.1 | `bmm_E2m1` (swiGlu) | 32.1 | 0.84x |
| MoE down GEMM (gemm2) | `flydsl_moe2_afp4_wfp4` | 46.8 | `bmm_Bfloat16_E2m1` | 26.2 | 0.56x |
| MoE combine/finalize | `moe_reduction_kernel` | 22.2 | `finalizeKernelVecLoad` (fuses routed-scale) | 15.8 | 0.71x |
| MoE act quant | `dynamic_per_group_scaled_quant` (mxfp4) | 4.0 | `NVFP4Quantize` | 2.6 | 0.65x |
| **MoE subtotal** | | **111.1** | | **76.7** | **0.69x** |
| Dense GEMM | Tensile `MT256x256` | 38.7 | `nvjet_sm100 128x256` | ≈40 | |
| Dense GEMM | Tensile `MT224x256` | 23.0 | `nvjet_sm100 176x128` | ≈22 | |
| Dense GEMM | `aiter::bf16gemm_256x256` (q_b_proj ‡) | 11.5 | `nvjet_sm100 256x128` | ≈12 | |
| Dense GEMM | `hgemm_bf16_128x128` (router gemm) | 4.6 | — | — | |
...
| RMSNorm | `aiter::add_rmsnorm_quant` (norm+quant fused) | 15.9 | `fused_add_rmsnorm` (norm only; quant in MoE) | 14.7 | |
| **RMSNorm subtotal** | | **15.9** | | **14.7** | **0.92x** |
| **TOTAL prefill** | | **~375** | | **331** | **0.88x** |
