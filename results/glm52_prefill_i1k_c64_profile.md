...
- **Attention (0.67x, dense FA 27 vs 9.9): no viable fork lever — hard register-pressure wall on CDNA4.** ck_tile FA = 256 VGPR / 0 AGPR / 17 KB LDS → ~2 waves/SIMD; roofline 14% compute / 44% BW → latency-bound. The `[BM,256]` fp32 online-softmax accumulator needs ~256 VGPR; B200 escapes via TMEM, CDNA4 can't. Config levers exhausted: tiling sweep (best 1.00x), backend swap (1.01x), `waves_per_eu` 1→8 (env `SGLANG_FA_WAVES_PER_EU`, commit `4811ac0906`) all neutral-or-worse (wpe≥3 spills → 0.64x–0.07x). Only remaining paths are algorithmic accumulator reductions (bf16 O-accum / split-D 2-pass) — compute/accuracy trade-offs, unproven.
  - *fp8 batch-prefill (env `SGLANG_DSA_FP8_DENSE_ATTN`, default off; commits `c410a22111`/`9c966c21a0`/`6cea8561a2`): dead lever.* FA kernel itself is faster (isolated: 1.13–1.40x), and accuracy is parity (1319-q GSM8K 0.928 vs 0.929). But `kv_b_proj` is bf16 so q/k/v must be re-quantized every forward; even after fixes (per-forward paging cache + fused `scaled_fp8_quant`) e2e is **+6/+42/+25%** at conc 4/8/16 and **OOMs at conc32+** under the required 0.85 static. The fused fp8-K/V path needs MXFP4 `kv_b_proj` (structurally excluded for GLM — decode absorb + faults, `SGLANG_FORCE_MXFP4_KVB` experiment).
- **MoE (0.78x):**
