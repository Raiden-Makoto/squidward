...
- **Anomaly: the router GEMM costs 4.1 ms/forward for no good reason.** Most dense GEMMs are at parity with B200, but the MoE router/gate GEMM (`hgemm_bf16_128x128` via `aiter_dsv3_router_gemm`, shape M×6144→256) takes **4.1 ms/forward** — far too much for such a skinny (N=256) output. That's an inefficient kernel/tile pick for the router shape (B200 folds it into a negligible splitK gemm). Worth investigating: a better small-N GEMM (or fusing the router with the topk) should reclaim most of it.
