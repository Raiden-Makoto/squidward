...
- **Attention (0.67x, dense FA 27 vs 9.9): no viable fork lever found — fp8 batch-prefill is accuracy-neutral but net-negative e2e.** The bf16 ck_tile FA is occupancy-limited by the `[BM,256]` fp32 online-softmax accumulator in VGPRs (B200 holds it in TMEM → ~96% of ~2.25 PFLOPS bf16 vs MI355X ~14% util). Two fork-side experiments:
  ...
  - *fp8 batch-prefill (IMPLEMENTED, env-gated `SGLANG_DSA_FP8_DENSE_ATTN`, default off, commit `c410a22111`): accuracy-neutral but **net-negative e2e as-implemented** — the FA-kernel win is swamped by quant+paging overhead.* `aiter.mha_batch_prefill_func` takes fp8 q/k/v + descales; page_size=16 is the only D=256-supported variant (32/64/128/256 → `no matching kernel`). **FA kernel alone is faster** — microbench 1.526 → 1.334 ms = 1.14x; live prefill profile shows the fp8 `FmhaBatchPrefillWithPagedKVCache` at 4.9 ms/fwd. **But GLM-5.2's `kv_b_proj`/`q_b_proj`/`o_proj` are bf16 (quark `exclude`, not MXFP4/uint8), so the fused `fused_gemm_afp4wfp4_split_cat` fp8-K/V path (`forward_mha.py:299`) never fires;** `_forward_standard_mha_fp8` must quantize the decompressed bf16 K/V/Q to fp8 and scatter the varlen K/V into the page_size=16 paged layout — all as **unfused eager PyTorch ops**. Profile shows this adds ≈13–15 ms/fwd (scatter `index_elementwise` ~3.5 + `elementwise` ~5.8, amax `reduce` ~3.6 + `AbsFunctor` ~1.3, `float8_copy` ~1.7, `Memset` ~0.9), which scales with token count and dominates.
    - **Accuracy A/B (full 1319-q GSM8K, same box): fp8 = 0.928 vs bf16 = 0.929 → parity** (the earlier 200-q 0.930-vs-0.955 gap was noise).
    - **e2e TTFT A/B (i1k/o1k, MI355X TP4): fp8 is net-negative**, and a follow-up optimization (commit `fa56bcdd1b`: drop the page-16 scatter for 16-aligned seqlens, fuse amax via `aminmax`) recovered only ~3% at conc64 — i.e. **the scatter was NOT the dominant cost**; the isolated 1.14x FA-kernel win simply does not translate to real packed prefill batches (per-forward fp8 quant of q/k/v + the page-16 paged kernel underperform the contiguous bf16 ck_tile in-situ):

| conc | bf16 dense TTFT (ms) | fp8 (scatter) | fp8 (optimized) | Δ opt vs bf16 |
| --- | ---: | ---: | ---: | ---: |
| 4  | 201.7 | 228.0 | 227.1 | +13% |
| 8  | 264.7 | 511.3 | 452.5 | +71% |
| 16 | 478.8 | 638.3 | 645.7 | +35% |
| 32 | 798.2 | 1135.1 | 1126.3 | +41% |
| 64 | 1186.8 | 2130.8 | 2061.7 | +74% |

    - **Verdict: dead lever for this checkpoint; kept default-off.** Two rounds (scatter-free + fused amax) left it +74% at conc64, so this is not an "overhead I can shave" problem — the fp8 `mha_batch_prefill` (page-16) path is net-slower than contiguous bf16 ck_tile at real packed prefill batches, and the per-forward q/k/v fp8 quant is pure added cost since GLM-5.2's `kv_b_proj` is bf16 (no pre-fp8/paged K/V; that would need MXFP4 `kv_b_proj` + the fused `fused_gemm_afp4wfp4_split_cat` path, which our checkpoint doesn't have). The isolated 1.14x FA microbench was unrepresentative. B200 side `fmhaSm100f...` is a trtllm-gen precompiled cubin (no portable source).
