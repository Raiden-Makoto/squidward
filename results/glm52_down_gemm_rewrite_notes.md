...
**S=3 is the optimum at nk=8, numerically exact** — BUT see the deployment-shape caveat below.
Committed to the aiter fork branch `glm52-moe2-pipe3` (`0fd2d2bb9`, off box baseline
`9127c94a1`), env-gated `SGLANG_MOE2_PIPE` (default 2 = no change).

## CRITICAL — pipe3 does NOT apply to the deployed TP4 down-GEMM (nk=2)

The -4.7% above was measured at `inter_dim=2048` (nk = 2048/256 = 8). **That is the UNSHARDED
shape.** On the real TP4 server, MoE `inter_dim` is row-parallel sharded to 512/rank, so the
down-GEMM K = 512 and with `tile_k=256` there are only **nk=2 K-tiles per rank**. Confirmed on
the live server (`[PIPE3DBG]` in the worker log): the production prefill kernel compiles as
`t64x128x256 pipe_deep=False S=2 nk=2`. `SGLANG_MOE2_PIPE=3` DOES reach the workers (SGLANG_*
is forwarded), but the gate (`nk >= S`) correctly disables it — **pipe3 is a no-op in
deployment**, and the profile doc's "deepen 2->3/4-stage" lever is infeasible (can't deepen a
2-tile loop).

Empirical confirmation at the correct per-rank shape (`-dim 6144,512 -t 16384`, forced
`reduce_persist_sbm128`, GPU-isolated): moe2 **S=2 = 884.1 us vs S=3 = 900.3 us** (identical;
+1.8% is run noise, pipe_deep gated off at nk=2). True per-rank down-GEMM baseline is **~884 us**
(K=512), not the 2619 us measured at the unsharded K=2048.

Consequences:
- The op_test microbench MUST use the per-rank shape `-dim 6144,512` (nk=2), NOT `6144,2048`.
- At nk=2 the mainloop is tiny; the down-GEMM is overhead/epilogue-bound (13% MfmaUtil), which
  ties it to the combine/finalize HBM round-trip. The real lever is fusion (PDL-like), already
  scoped as high-effort/defer in the profile doc — NOT K-pipelining.
