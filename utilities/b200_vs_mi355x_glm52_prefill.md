...
| Sparse-MLA prefill attn (`_sparse_mla_fwd`, #28975) | 494 vs 410 ms | Triton stays; gluon proven unfit. Built a correct gluon rewrite (harness maxdiff 0.0004) after clearing the CDNA4 compiler wall (fp8 needs `mfma_scaled` + `[32,32,64]`/k_width=16, not plain mfma). But measured 30 ms/call (tl_dot blocked-load+convert) and 92 ms/call (direct dot-layout loads) vs triton 2.0 ms -- 15-45x slower. gluon's MFMA-layout machinery is counterproductive for this small-M (16 heads) memory-bound gather; competitive perf would need a shared-memory async-copy scatter-gather pipeline (multi-day, uncertain feasibility for scatter, bounded <=~15% since triton is already ~4.8 TB/s near DRAM peak). Conclusion: triton #28975 is near-optimal for this shape; the 84 ms vs B200 is HBM-BW + tcgen05 hardware. Gluon artifact kept flag-gated (SGLANG_DSA_SPARSE_MLA_GLUON, default off) for future LDS-pipeline work. |
