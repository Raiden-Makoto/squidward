...
| Sparse-MLA prefill attn (`_sparse_mla_fwd`, #28975) | 494 vs 410 ms | Near ceiling, ours. VERIFIED: at seq=8192 the kernel is 1.732 ms/call and its logical gather BW (5.58 TB/s) is 123% of measured HBM copy peak (4.55 TB/s) -> overlapping-topk KV is already served from L2, so it's cache/BW-bound near ceiling and the M=16 MFMA shape caps compute. Query-block reuse is already captured by L2; a gluon rewrite is ~10-15% at most, not worth it. 494-vs-410 gap is HBM-BW + hardware generation. Keep #28975 as-is. |
