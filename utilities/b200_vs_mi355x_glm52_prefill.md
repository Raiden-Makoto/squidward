...
| **Sparse-MLA prefill attn (`_sparse_mla_fwd`, #28975)** | **494 vs 410 ms** | **OPEN, best target, 100% ours.** Biggest single prefill kernel. Plain Triton, memory-bound on per-query KV gather (~9.7 GB/launch at seq=8192). Bandwidth floor ~1.8 ms/call vs measured 3.17 ms/call -> ~200 ms overhead headroom. Levers: (1) gluon rewrite (async buffer_load KV-gather pipeline + swizzled LDS + mfma_scaled; ~2.4x proven on the logits kernel), (2) query-block KV reuse (amortize overlapping topk gathers). PENDING: confirm memory-vs-compute split before starting. |
