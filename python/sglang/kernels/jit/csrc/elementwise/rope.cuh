...
// CacheType is the cos_sin_cache element type; only the interleaved path reads
// it as anything other than float.
template <typename T, bool kIsNeox, int64_t kRopeDim, int64_t kVecSize, typename CacheType = float>
...
__global__ void fused_rope_kernel(const __grid_constant__ FusedRopeParams params) {
  using namespace device;
  static_assert(
      kIsNeox ? std::is_same_v<CacheType, float> : true,
  ...
  for (auto idx = start_worker_id; idx < num_works; idx += num_workers) {
    ...
    if (load_q) {
      rope_rotate_head<QDType, kIsNeox, kRopeDim, kVecSize, CacheType>(input, cos_ptr, sin_ptr, lane_id);
    } else {
      rope_rotate_head<DType, kIsNeox, kRopeDim, kVecSize, CacheType>(input, cos_ptr, sin_ptr, lane_id);
    }
  }
...
// dtypes only (fp16 q with bf16 k for the fp8-KV fp16-Q feed). CacheType is the
// cos_sin_cache dtype (interleaved path only).
template <
    bool kIsNeox,
    int64_t kRopeDim,
    bool kUsePDL,
    typename DType,
    typename QDType = DType,
    typename CacheType = float>
struct FusedRopeKernel {
  ...
  static constexpr auto _kernel_0 =
      fused_rope_kernel<kIsNeox, kRopeDim, kUsePDL, DType, QDType, IdType, kWorkThreads, CacheType>;
  template <typename IdType>
