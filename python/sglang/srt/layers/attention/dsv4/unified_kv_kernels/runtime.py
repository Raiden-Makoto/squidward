...
# RoPE8 combined row: [0:512] fp8 (NoPE 0:448 + RoPE 448:512), [512:528] E8M0.
# E8M0 lives INLINE right after the fp8 data (not in a separate buffer) so the
# decode E8M0 read shares cache lines with the fp8-tile gather (a separate small
...
def _mxfp8_rope8_pack_dense_kernel(
    kv_ptr,  # [N, head_dim] norm+rope'd (any float dtype)
    comb_u8_ptr,  # [N, ROW_WIDTH] uint8 combined out (fp8 0:512 + E8M0 512:528)
    n_rows,
    kv_row_stride,
    kv_col_stride,
    ...
    FP8_BLK: tl.constexpr,  # 32
    SCALE_OFF: tl.constexpr,  # 512
    ROW_WIDTH: tl.constexpr,  # 528
    FP8_MAX: tl.constexpr,  # 448.0
    BLOCK_ROWS: tl.constexpr,
):
    """RoPE8 dense packer: quantize the FULL head latent (NoPE 0:448 + RoPE
    448:512) to MXFP8 (E8M0 per-32-block) into a contiguous [N, 528] uint8 row:
    fp8 data at cols 0:512, E8M0 exponents inline at cols 512:528. Identical
    per-32 block math to the split packer; the RoPE tail is quantized in-place
    (blocks 14,15) instead of copied as bf16, and the E8M0 bytes sit right after
    the fp8 data so the decode read keeps them cache-line-local."""
    pid = tl.program_id(0)
    ...
    for b in tl.static_range(NUM_BLOCKS):
        ...
        qb = q.to(tl.uint8, bitcast=True)
        tl.store(
            comb_u8_ptr + rows[:, None] * ROW_WIDTH + cols[None, :],
            qb,
            mask=row_mask[:, None],
        )
        e_byte = e_unb.to(tl.int32) + 127
        e_byte = tl.minimum(tl.maximum(e_byte, 0), 255).to(tl.uint8)
        tl.store(comb_u8_ptr + rows * ROW_WIDTH + SCALE_OFF + b, e_byte, mask=row_mask)


def pack_mxfp8_rope8_dense(
    x: torch.Tensor,  # [..., head_dim] norm+rope'd (any float dtype)
) -> Tuple[torch.Tensor, torch.Tensor]:
    """RoPE8 dense pack: returns ``(kv_fp8 [..., 512], scale [..., 16])`` -- both
    are strided views into ONE contiguous ``[..., 528]`` uint8 buffer (fp8 data
    0:512, E8M0 inline 512:528). Co-located layout for the fused 2-gather decode
    read (``SGLANG_UNIFIED_KV_FP8_ROPE8``); keeping E8M0 inline preserves
    cache-line locality with the fp8 tile load."""
    from sglang.srt.layers.attention.dsv4.unified_kv_kernels import mxfp8
    ...
    flat = x.reshape(-1, mxfp8.DIM_HEAD)
    n_rows = flat.shape[0]
    comb = torch.zeros(n_rows, ROPE8_ROW_WIDTH, dtype=torch.uint8, device=x.device)
    if n_rows > 0:
        BLOCK_ROWS = 8
        grid = (triton.cdiv(n_rows, BLOCK_ROWS),)
        _mxfp8_rope8_pack_dense_kernel[grid](
            ...
            FP8_BLK=mxfp8.FP8_BLOCK,
            SCALE_OFF=ROPE8_SCALE_OFF,
            ROW_WIDTH=ROPE8_ROW_WIDTH,
            FP8_MAX=mxfp8.FP8_MAX,
            BLOCK_ROWS=BLOCK_ROWS,
            num_warps=4,
        )
    kv_fp8 = comb[:, : mxfp8.DIM_HEAD].view(mxfp8.FP8_DTYPE).reshape(
        *lead, mxfp8.DIM_HEAD
    )
    scale = comb[:, ROPE8_SCALE_OFF : ROPE8_SCALE_OFF + num_blocks].reshape(
        *lead, num_blocks
    )
    return kv_fp8, scale
